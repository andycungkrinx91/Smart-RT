...
export async function GET(_req: Request, context: { params: Promise<{ nomor: string }> }) {
  ...
  try {
    const response = await fetch(`${EQURAN_BASE_URL}/tafsir/${nomor}`, {
      method: "GET",
    ...
    if (!response.ok) {
      return NextResponse.json({ message: "Failed to fetch tafsir detail" }, { status: response.status });
    }
    ...
    return NextResponse.json({ message: "Tafsir service unavailable" }, { status: 502 });
  }
}
