...
export default function BottomNav() {
  ...
  return (
    <nav className="fixed inset-x-0 bottom-0 z-50 lg:hidden" aria-label="Navigasi bawah">
      <div className="relative w-full">
        <AnimatePresence>
          {openMenu && (
            <motion.div
              key={openMenu.id}
              className={`absolute bottom-full mb-3 z-20 w-[min(92vw,22rem)] ${openMenu.side === 'left' ? 'left-2' : 'right-2'}`}
        ...
        <motion.div
          ...
          className="grid w-full grid-cols-[minmax(0,1fr)_minmax(0,1fr)_4.75rem_minmax(0,1fr)_minmax(0,1fr)] items-end gap-2 rounded-t-2xl border-t border-slate-800 bg-[#0a1020] px-2 pb-1 pt-0 shadow-[0_-8px_28px_rgba(0,0,0,0.4)]"
        >
          <Link
            href="/blogs"
            aria-current={isBlogActive ? 'page' : undefined}
            className={`group relative flex min-h-[4.5rem] min-w-0 flex-col items-center justify-center gap-2 rounded-xl px-2 text-[12px] font-semibold transition ${
              isBlogActive
                ? 'bg-[var(--color-brand)] text-white shadow-lg'
                : 'text-slate-400 hover:bg-white/10 hover:text-slate-200'
            }`}
          >
            <Newspaper className={`h-7 w-7 ${isBlogActive ? 'text-white' : 'text-slate-400'}`} aria-hidden="true" />
            <span>Blog</span>
          </Link>
          ...
          >
            <FiturIcon className={`h-7 w-7 ${isFiturActive ? 'text-white' : 'text-slate-400'}`} aria-hidden="true" />
          ...
          <Link href="/" aria-current={isDashboardActive ? 'page' : undefined} className="relative -mt-7 flex flex-col items-center justify-end gap-2">
            <motion.span
              whileTap={{ scale: 0.92 }}
            ...
            <span className={`text-[13px] font-semibold ${isDashboardActive ? 'text-[var(--color-brand)]' : 'text-slate-400'}`}>Beranda</span>
          </Link>
          ...
          >
            <Clock3 className={`h-7 w-7 ${isShalatActive ? 'text-white' : 'text-slate-400'}`} aria-hidden="true" />
          ...
          >
            <SettingIcon className={`h-7 w-7 ${isSettingActive ? 'text-white' : 'text-slate-400'}`} aria-hidden="true" />
